...
说明：
  - 预测：DensityPredictor 用最近 window_size 帧预测未来 pred_horizon 分钟密度
...
class SecurityService:
    ...
    model_dir : str
        训练产出的模型目录（含 model_state.ckpt + config.json）。
    ...
    def _build_feature_matrix(self, df):
        ...
        for f, col in enumerate(self.state_features):
            ...
            X_state[:, :, f] = piv.to_numpy(dtype=np.float32)

        if np.isnan(X_state).any():
            raise ValueError("透视后存在 NaN，数据不完整")
    ...
    def predict(self):
        """
        读取 CSV 最新数据，预测未来 pred_horizon 分钟各节点密度。
        ...
        pred_start = last_ts + pd.Timedelta(seconds=60)
        pred_end = last_ts + pd.Timedelta(minutes=self.pred_horizon)
    ...
    def check_alerts(self):
        ...
        pred = {
            "timestamp": str(last_ts),
            "period": {
                "start": str(last_ts + pd.Timedelta(seconds=60)),
                "end": str(last_ts + pd.Timedelta(minutes=self.pred_horizon)),
            },
            "density_stats": density_stats,
        }
        ...
        while True:
            try:
                ...
                if result.get("skipped"):
                    ...
                    print(f"  预测时段：{period['start']} ~ {period['end']}（未来{self.pred_horizon}分钟）")
