# -*- coding: utf-8 -*-
"""
alert.py — 安全预警与应急响应脚本（MindSpore 版本，含 DensityPredictor 联动）
=============================================================================
在 VSCode 中直接点击右上角「运行」按钮即可执行。

流程：加载 DensityPredictor + CongestionDetector 基线 → 读最新数据
      → 用最近 window_size 帧预测未来 → 检测最近 RECENT_FRAMES 帧 + 用预测确认拥堵
      → 分级 + 补字段 → 打印入库 JSON + 保存文件 → POST 后端
...
# 2. 模型 / 归一化参数目录（MindSpore train.py 运行后自动生成）
...
def load_data():
    """
    读取 CSV，构造与 train.py 一致的完整特征矩阵 (n_ticks, n_nodes, n_features)。
    特征维数 = 10 状态特征 + 4 时间特征，按 preprocessor 的节点顺序对齐。
    返回 (X_raw, X_norm, node_ids, density_col_idx, norm_min, norm_max, tick_to_ts)
    """
    ...
    state_features = feature_columns[:state_feature_count]   # 前 N 列状态特征
...
def login_get_token():
    """调用后端登录接口获取 Bearer token。失败返回 None。"""
    import requests
    try:
        resp = requests.post(
            LOGIN_URL,
...
def main():
    print("=" * 60)
    print("安全预警与应急响应（MindSpore / 预测联动）")
    print("=" * 60)
    ...
    if len(alerts) == 0 and DEMO_MODE:
        ...
        print(f"  演示预警 node_id={demo_node}")
    elif len(alerts) == 0:
        print("\n  当前无预警（L0 正常），未发送任何请求。")
        return
    ...
    for a in alerts:
        ...
        print(f"      预计持续  : {a['predicted_duration_min']} 时间步")
