...
说明：
  - gate_status / door_status  : 门闸状态，字符串 open / close
...
# 门闸状态字符串 → 数值（编码成数值喂模型）
# 实测数据取值：open / restricted / closed（空值统一填 0）
...
STATE_FEATURES = [
    ...
    "people",             # 1
    "vehicles",           # 2
    "gate_status",        # 3 编码 open→1 close→0
    "gate_flow_rate",     # 4
    "level",              # 5 编码 low→1...critical→4
    "door_status",        # 6 编码 open→1 close→0
    "door_flow_rate",     # 7
