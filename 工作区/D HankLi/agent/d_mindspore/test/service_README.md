...
| 等级 | 触发条件 | 阈值来源 | 预计持续兜底 |
|---|---|---|---|
| **L1 关注** | 密度超 P85 未达 P95（连续 3 帧） | `threshold_p85` | 5 时间步（50秒） |
| **L2 预警** | 密度超 P95 / 滞留 / 门闸异常 | `threshold_p95` | 5 时间步（50秒） |
| **L3 严重** | 密度超 1.5×P95 / 滞留超 2×时长 / 门闸故障 | `threshold_p95` | 10 时间步（100秒） |

**防误报机制：**
- **天然空节点过滤**：节点 P95 阈值 < 0.05（天桥/地下通道/交叉口等历史上几乎无人）不参与判定，避免微小波动误报
- **连续帧确认**：L1 也要连续 3 帧才报，过滤瞬时抖动
