...
读取 demo_platoon_edge.py 产出的帧/遥测数据，生成单文件 fleet_animation.html：
- 主图：路网 + route_nodes 高亮 + 编队车（leader/跟随）逐帧行驶
- 控制条：播放/暂停 / 倍速 / 时间轴滑块 / 当前 tick
- 遥测面板：速度、车-车间距、前车到目标节点距离 三条曲线（与播放头联动 markLine）

用法：
    cd 项目目录
    python animate_fleet.py --topology data/platoon_topo.yaml \
        --input data/platoon_frames.json --telemetry data/platoon_telemetry.csv \
        --out platoon_animation.html
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="note">黄色粗线 = 高亮路线 · 大圆点 = leader（CAV_L1）</div>
  </div>
...
const allTicks = frames.map(f => f.tick);
...
const pf = firstFrame.path || {};
const routeLen = firstFrame.total_path_length || 0;
document.getElementById('titleO').textContent = (pf.start_node_id||'-') + ' → ' + (pf.end_node_id||'-');
document.getElementById('subO').textContent =
  '数据源: ' + (meta.source || 'platoon_frames.json') + ' · ' + (meta.fleet_size||'-') + ' 车编队 · ' +
  frames.length + ' 帧（tick ' + (allTicks[0]??'-') + '→' + (allTicks[allTicks.length-1]??'-') + '）' +
  ' · 路径总长 ' + routeLen + ' m';
...
let routeCoords = [];
if (pf.route_nodes && pf.route_nodes.length) {
  routeCoords = pf.route_nodes.map(id => nodePos[id] ? [nodePos[id][0], nodePos[id][1]] : null)
                              .filter(Boolean);
}
    ...
    series: [
      ...
      { id: 'cars', name: 'cars', type: 'effectScatter', coordinateSystem: 'cartesian2d',
        ...
        label: { show: true, position: 'top', color: '#fff', fontSize: 11,
                 textBorderColor: '#000', textBorderWidth: 1 } }
    ]
  };
}
...
def main(argv=None):
    ...
    p.add_argument("--input", default="data/platoon_frames.json")
    p.add_argument("--telemetry", default="data/platoon_telemetry.csv")
    p.add_argument("--topology", default="data/platoon_topo.yaml")
    ...
    if not frames.get("frames"):
        raise SystemExit("帧文件为空，请先运行 demo_platoon_edge.py 生成数据")
    if not tele:
        print("警告: 遥测 CSV 为空，曲线面板将无数据")
    ...
    print(f"  拓扑 {len(topo['nodePos'])} 节点 / {len(topo['edges'])} 边 | "
          f"帧 {len(frames['frames'])} | 遥测车 {list(tele.keys())}")
