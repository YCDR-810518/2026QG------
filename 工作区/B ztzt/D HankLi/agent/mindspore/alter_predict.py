# -*- coding: utf-8 -*-
"""
alter_predict.py — 指定时段密度预测脚本（MindSpore 版本）
==========================================================
在 VSCode 中直接点击右上角「运行」按钮即可执行。

作用：预测指定时段（如"周一早10点"、"周五晚6点"）未来密度的各节点密度。

与 predict.py 的区别：
  - predict.py 只预测数据末尾的"当前"时刻
  - alter_predict.py 从数据中选取任意目标时段，取该时段之前 window_size 个时间步做输入，
    预测该时段之后 pred_horizon 个时间步。支持一次配置多个时段。

流程：加载模型 → 读取数据 → 构造特征 → 对每个目标时段取前 window_size 步
      → 预测未来 pred_horizon 步 → 汇总成多个 density_stats
"""
...
# 6. 计算设备（None 自动检测：GPU/Ascend/CPU）
DEVICE = None
...
def build_feature_matrix(df, ticks, node_ids, state_features, state_feature_count):
    ...
    状态特征来自 CSV（空值填 0），时间特征由 timestamp 派生。
    """
...
def main():
    print("=" * 60)
    print("指定时段密度预测（alter_predict / MindSpore）")
    print("=" * 60)
    ...
    output = {
        ...
        "note": f"指定时段密度预测，每个时段用其前 {window_size} 个时间步预测未来 {pred_horizon} 个时间步",
