...
流程：加载 train.py 训练好的模型 → 读取当前数据 → 取最近一段历史
      → 预测未来 10 分钟各节点密度 → 输出 Dict[node_id, 密度]
...
# 4. 节点ID → 区域名 的映射（可选）。
#    例如 {"P01": "zone_canteen", "G01": "gate_north_01"}；
#    不填或没匹配到的节点，输出直接用节点ID作为 key。
NODE_NAME_MAP = {}

# 5. 计算设备（None 自动选：cuda > mps > cpu）
DEVICE = None
...
def main():
    ...
    # ---- 时间特征：hour_sin/hour_cos/dow_sin/dow_cos（与 train.py 相同） ----
    ...
    X_time = np.repeat(time_feat[:, None, :], len(node_ids), axis=1)  # (n_ticks, 节点, 4)
    ...
    # 归一化（用训练时保存的 min/max，对每个被归一化的列操作）
    ...
    # 逆归一化，还原成真实密度单位（与 train.py 评估时一致）
    lo = norm_min[density_col_idx]
    hi = norm_max[density_col_idx]
    y_pred = y_pred_norm * (hi - lo + 1e-8) + lo                    # (1, 节点, 预测步长)
    y_pred = y_pred[0]                                              # (节点, 预测步长)
    y_pred = np.clip(y_pred, 0.0, None)                             # 密度不可能是负数，兜底裁剪

    # ---------- 5. 汇总成 Dict[node_id, 密度] ----------
    print("\n[5/5] 汇总输出 ...")
    if PRED_AGG == "max":
        agg_values = y_pred.max(axis=1)          # 未来 30 秒峰值
    elif PRED_AGG == "mean":
        agg_values = y_pred.mean(axis=1)         # 未来 30 秒平均
    elif PRED_AGG == "first":
        agg_values = y_pred[:, 0]                # 未来第 1 个时间步
    else:
    ...
    # 预测时间段：从最后一个观测时刻开始，覆盖未来 pred_horizon 个时间步
    last_ts = tick_to_ts.iloc[-1]                 # 最后一个观测 tick 的时刻
    pred_start = last_ts + pd.Timedelta(seconds=TICK_SECONDS)                # 预测的第一个采样点
    pred_end = last_ts + pd.Timedelta(seconds=pred_horizon * TICK_SECONDS)   # 预测覆盖到的时刻
