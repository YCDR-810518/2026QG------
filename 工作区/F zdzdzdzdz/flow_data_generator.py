...
class FlowDataGenerator:
    ...
    sample_interval : int
        密度采样间隔（秒），默认 60。
    start_hour / n_hours : int
        模拟起点时刻 / 时长（小时），默认 6 点起共 16 小时。
    n_days : int
    ...
    def __init__(self, n_people=4000, n_vehicles=300, density_level="peak",
                 ...
                 sample_interval=60, start_hour=6, n_hours=16,
    ...
    def get_params(self, deep=True):
        return {
            ...
            "n_hours": self.n_hours, "n_days": self.n_days,
    ...
    def _sample_entities(self, n_entities, kind, internal_prob, curves):
        ...
        birth_tick 为"距当日 06:00 的秒数"（0 ~ n_hours*3600-1），跨天偏移由调用方叠加。
        """
        n_min = self.n_hours * 60
        peaking = {"off_peak": 0.8, "peak": 2.0, "ultra_peak": 3.0}[self.density_level]

        hour_grid = self.start_hour + (np.arange(n_min) * 60 / 3600.0)
        w = np.empty((self.n_nodes, n_min), dtype=np.float64)
        for i in range(self.n_nodes):
            w[i] = np.interp(hour_grid, np.arange(24), curves[i])
        ...
        births = (mins_out * 60 + self._rng.uniform(0, 60, size=n_src).astype(np.int64))
    ...
    def generate(self):
        ...
        for d in range(self.n_days):
            ...
            pb, ps, pd, _ = self._sample_entities(self.n_people, kind=0, internal_prob=0.0, curves=curves)
